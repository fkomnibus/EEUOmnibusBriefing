...
fig.update_layout(
    ...
    xaxis=dict(tickfont=dict(color='darkblue'),
        title_font=dict(color='darkblue'),tickformat='%Y-%m'),
    hovermode='x unified',
    yaxis=dict(
...
```{=html}
<!-- This injects the graph into your page -->
...
```{=html}
<script>
  // Listen for clicks on the Plotly chart
  document.getElementById('graph').on('plotly_click', function(data) {
    ...
    // Optionally trigger the calculator if emissions are already filled
    calculateCBAM();
  });
</script>
```